...
def rank_resumes():
    ...
    try:
        ...
        if not saved_resume_paths:
            return jsonify({
                'status': 'error',
                'message': 'No valid PDF resumes provided'
            }), 400
        ...
        try:
            ...
            return jsonify({
                'status': 'error',
                'message': f'Failed to parse resumes: {str(e)}'
            }), 400
        
        if not parsed_resumes:
            return jsonify({
                'status': 'error',
                'message': 'Failed to parse any resumes'
            }), 400
